...
| 模块 | 功能 |
| --- | --- |
| 统一工作区 | 顶部提供全局搜索、批量任务、模型库和设置；左侧只保留“素材、歌词与标签、音频编辑、导出与传输”四个任务入口；AI 仅在启动后从右侧展开。 |
| 素材库 | 音乐、视频两种互斥模式；文件夹独立保存；紧凑分段切换与多列浏览，列表优先显示封面、名称、格式、状态和大小，选择素材后再启用歌词、封面或编辑入口。 |
...
| 音频编辑 | 三栏工作区集中提供提取、裁剪、拼接、混音、录音、淡入淡出、变速变调、降噪、响度归一化、分割、均衡器、音量、标签、格式转换和倒放；左侧工具和右侧参数可独立滚动，中间素材、波形和播放区始终保留。 |
| GPU | 自动诊断显卡与运行时；可安装并启用匹配的外置 GPU 推理运行时，无法使用时安全回退 CPU。 |
| 视频时间 | 读取视频时间元数据；左、右时间可精确编辑到秒；使用小时偏移快速校准并导出视频文字时间轴。 |
| 歌词与封面 | 生成、繁转简、编辑、翻译、LRCLIB 歌词匹配与 AI 校准；从 MusicBrainz/Cover Art Archive 匹配封面或选择本地图片并写入音频标签。 |
| 密钥管理 | 从顶部“设置”集中管理 Groq、讯飞和 DeepSeek Key；Key 仅保存在当前 Windows 用户的本机配置中。 |
| AI 助手 | 默认隐藏，启动后从右侧展开；可切换 DeepSeek 或本地 OpenAI 兼容模型，并在每次对话中加载内置使用手册与系统提示词。 |
...
点击顶部“设置 → 启动 AI 助手”。在线模式需先填写 DeepSeek Key；本地模式在“设置 → 本地部署 AI”中选择 Ollama、LM Studio 预设或自定义 OpenAI 兼容接口，并填写模型名称，本地 Key 可留空。AI 会从主窗口右侧展开；再次点击“设置 → 关闭 AI 助手”即可收起。
...
切到左侧“歌词与标签”，再选择“在线歌词与封面”时，会显示完整歌词核对区：左半栏是本地识别歌词，右半栏是自动繁转简后的在线歌词，下方是本地素材播放器。搜索歌词、搜索封面和本地封面共用结果区；点击封面后确认即可写入音频标签，不再需要额外的写入按钮。素材列表会在歌曲名称旁显示内嵌封面缩略图。参见 [在线歌词匹配与 AI 校准指南](docs/在线歌词匹配与AI校准指南.md)。
...
选择素材后打开左侧“音频编辑”。页面保持当前素材不变，左侧选择工具，中间显示素材、
波形和播放控制，右侧显示当前工具参数：

- 基础剪辑：提取音轨、时间裁剪、首尾拼接、多轨混合、固定时长分割。
- 听感处理：淡入淡出、音量增益、响度归一化、FFT 降噪和三段均衡器。
- 变换输出：速度与音高、声道/采样率/格式转换、倒放。
- 文件能力：Windows 麦克风录音、常用音乐标签编辑、输入/输出试听和结果管理。

默认在原素材旁建立 `Echovault编辑输出`，所有操作生成新文件，不覆盖原素材。若输入来自
手机传输任务，编辑结果会自动复制或硬链接到“待回传”目录，并刷新手机传输差异列表。
