...
| LRC | 生成、繁转简、编辑、时间戳调整、歌词预览、AI/Argos 翻译，以及 LRCLIB 在线匹配与 AI 文字校准。 |
...
切到右侧“在线匹配”时，主窗口左侧会换成完整歌词核对区：左半栏是本地识别歌词，右半栏是自动繁转简后的在线歌词，下方是本地素材播放器。右侧可按音乐库/视频库筛选全部已配置素材；顶部“开始识别/重新识别”在“搜索 LRCLIB”左边，下面只放结果表、直接应用、交叉合并和 AI 校准按钮。AI 校准使用左侧当前时间轴和右侧当前文字并直接刷新左侧；播放器按同一位置实时高亮、滚动两侧歌词，双击任一侧可暂停并修改。参见 [在线歌词匹配与 AI 校准指南](docs/在线歌词匹配与AI校准指南.md)。
