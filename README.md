...
| 音频编辑 | 专用波形编辑器支持任意常见媒体的振幅显示、游标、拖拽选区、精确起止时间、缩放和平移；效果面板提供裁剪与淡化、分段、增益、降噪、响度标准化、均衡、变速变调、拼接、混合、提取和标签。 |
...
选择素材后打开左侧“音频编辑”。页面保持当前素材不变，左侧选择工具，中间显示可缩放
波形、时间游标和编辑选区，右侧显示当前效果参数：

- 基础编辑：裁剪与淡化、分段导出、增益。
- 音质处理：FFT 降噪、响度标准化、三段均衡器、独立变速变调。
- 合成与输出：按顺序拼接、多轨混合、从视频提取音轨、常用音乐标签。

在波形中拖动即可创建选区；起点、终点和时长也可精确输入。支持选区的效果在有选区时
只生成选区结果，没有选区时处理整段。默认在原素材旁建立 `Echovault编辑输出`，所有操作
