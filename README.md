...
切到右侧“在线匹配”时，主窗口左侧会换成完整歌词核对区：左半栏是本地识别歌词，右半栏是当前在线歌词，下方是本地素材播放器。右侧“在线匹配”页只放歌曲/搜索条件、LRCLIB 结果表，以及本地识别、直接应用、交叉合并和 AI 校准按钮。选择结果后在线歌词显示到左侧核对区；可单独采用一侧，或组合“本地时间轴 + 在线文字”“在线时间轴 + 本地文字”。播放器按同一位置实时高亮、滚动两侧歌词；双击任一侧可暂停并修改。参见 [在线歌词匹配与 AI 校准指南](docs/在线歌词匹配与AI校准指南.md)。
