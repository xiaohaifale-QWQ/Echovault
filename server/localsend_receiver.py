...
class LocalSendReceiver:
    ...
    def start(self):
        ...
        self._start_browse_server()
        try:
            self._start_udp_multicast()
            self._send_announcement()
        except Exception as e:
            logger.warning(f"UDP failed: {e}")
    ...
    def stop(self):
        ...
        if self._browse_server:
            self._browse_server.shutdown()
        if self._udp_socket:
        ...
        class H(BaseHTTPRequestHandler):
            ...
            def do_POST(s):
                ...
                if p == "/api/localsend/v2/register":
                    ...
                    s._prep()
                elif p == "/api/localsend/v2/upload":
                    s._up()
                elif p == "/api/localsend/v2/cancel":
                    s._cancel()
                else:
                    s._json({"message": "not found"}, 404)

            def do_GET(s):
                if s.path.split("?")[0] == "/api/localsend/v2/info":
                    s._info()
                else:
                    s._json({"message": "not found"}, 404)
            ...
            def _prep(s):
                ...
                for fid, inf in fs.items():
                    ...
                    if ex.exists() and ex.stat().st_size == sz:
                        skipped += 1
                        continue
                    tk = uuid.uuid4().hex[:16]
                    tokens[fid] = tk
                    sf[fid] = {"name": nm, "size": sz, "token": tk}
                if skipped:
                    logger.info(f"skip {skipped} dups")
                if not sf:
                    s._empty(204)
                    return
            ...
            def _up(s):
                ...
                q = dict(urllib.parse.parse_qsl(s.path.split("?")[1])) if "?" in s.path else {}
                sid, fid, tok = q.get("sessionId", ""), q.get("fileId", ""), q.get("token", "")
                with r._session_lock:
                    se = r._sessions.get(sid)
                if not se:
                    s._json({"message": "bad session"}, 403)
                    return
                fi = se["files"].get(fid)
                if not fi or fi["token"] != tok:
                    s._json({"message": "bad token"}, 403)
                    return
                length = int(s.headers.get("Content-Length", 0))
                if length <= 0 or (fi["size"] and length != fi["size"]):
                    s._json({"message": "size mismatch"}, 400)
                    return
                if shutil.disk_usage(r.save_dir).free < length:
                    s._json({"message": "insufficient storage"}, 507)
                    return
                total, keys = len(se["files"]), list(se["files"].keys())
                idx = keys.index(fid) + 1 if fid in keys else 0
                fn = fi["name"]
                safe = Path(fn).name
                workspace = Path(se["workspace"])
                fp = workspace / safe
                if fp.exists():
                    st, su = fp.stem, fp.suffix
                    c = 1
                    while fp.exists():
                        fp = r.save_dir / f"{st} ({c}){su}"
                        c += 1
                ...
                try:
                    ...
                    s._json({"message": "incomplete upload"}, 400)
                    return
                except OSError as exc:
                    if temp_path.exists():
                        temp_path.unlink()
                    logger.error(f"upload failed: {exc}")
                    s._json({"message": "write failed"}, 500)
                    return
                logger.info(f"received: {safe} ({received}B)")
                completed = None
                with r._session_lock:
                    se["received"].add(fid)
                    se["paths"].append(str(fp))
...
<div class="card"><h2>发送到电脑</h2><p style="font-size:13px;color:#666;line-height:1.8">打开手机 LocalSend App，找到 <b>{r.alias}</b> 设备，选择文件发送。</p></div>
            ...
            def _dl(s, fn):
                ...
                s.send_header("Content-Length", str(fp.stat().st_size))
                s.end_headers()
    ...
    def _loop(self):
        while self._running:
            try:
                d, a = self._udp_socket.recvfrom(4096)
                self._on_udp(d, a)
            except OSError:
                if self._running:
                    logger.debug("LocalSend multicast receive failed", exc_info=True)
    ...
    def _on_udp(self, data, addr):
        ...
        if m.get("announce") and m.get("port"):
            self._reply(addr[0], m)
        ...
        try:
            ...
            urllib.request.urlopen(
                urllib.request.Request(
                    u, data=b, headers={"Content-Type": "application/json"}, method="POST"
                ),
                timeout=3,
                context=ctx,
            )
        except (OSError, ValueError, KeyError):
            logger.debug("LocalSend registration reply failed", exc_info=True)
    ...
    def _send_announcement(self):
        try:
            ...
            s.sendto(m, (MULTICAST_GROUP, MULTICAST_PORT))
            s.close()
        except Exception as e:
