...
def get_audio_info(file_path: str) -> dict:
    ...
    if not ffprobe_path:
        if Path(file_path).suffix.casefold() == ".wav":
            try:
...
def convert_to_whisper_format(
    ...
    args.extend(
        [
            "-vn",  # 视频素材只保留音轨
            "-ar",
            str(WHISPER_SAMPLE_RATE),  # 采样率 16kHz
            "-ac",
            str(WHISPER_CHANNELS),  # 单声道
            "-sample_fmt",
            "s16",  # 16-bit PCM
        ]
    )
    ...
    try:
        ...
        raise RuntimeError("未找到 ffmpeg。请先安装 ffmpeg 并确保 ffmpeg 命令已加入 PATH。") from e
