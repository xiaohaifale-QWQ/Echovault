...
“音频编辑”采用专用工作台：左侧选择裁剪与淡化、分段、增益、降噪、响度标准化、均衡器、变速变调、拼接、混合、提取音轨或标签；中间波形支持游标、拖拽选区、精确起止时间、缩放和平移，右侧切换当前效果参数。支持选区的效果在有选区时只生成选区结果，没有选区时处理整段；结果始终保存为新文件并可进入手机待回传流程。
