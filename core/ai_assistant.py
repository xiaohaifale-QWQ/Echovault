...
主窗口左侧只有四个任务工作区：素材、歌词与标签、音频编辑、导出与传输。AI 助手默认不占空间，启动 AI 模式后在标题栏和顶部操作区下方，从主内容区右侧展开为紧凑抽屉。素材工作区支持音乐和视频两种互斥模式、独立文件夹、多列文件浏览；选择文件后会给出歌词、封面/标签和音频编辑入口。未勾选“全选”时列表只显示当前目录；勾选后汇总本模式全部已添加目录。
