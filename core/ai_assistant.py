...
右侧“在线匹配”可从 LRCLIB 搜索公开歌词：下载同步歌词会先备份已有 LRC；AI 核对会保留本地时间戳，只校准歌词文字。该页也可从 MusicBrainz/Cover Art Archive 搜索封面，或选择本地 JPEG/PNG 写入常见音频格式的内嵌标签；素材列表会显示封面缩略图。
右侧“音频编辑”提供提取、裁剪、拼接、混音、录音、淡入淡出、变速变调、降噪、响度归一化、分割、均衡器、音量、标签、格式转换和倒放；每项都有独立参数页，结果保存为新文件并可进入手机待回传流程。
