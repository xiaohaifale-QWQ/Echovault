...
“歌词与标签”包含“在线歌词与封面、本地识别编辑、歌词核对”三个功能。“在线歌词与封面”排在第一位，顶部搜索卡可一键并行搜索 LRCLIB 歌词和在线封面；精确歌词先显示、完整候选后台补齐，封面优先走 Apple 公开目录，未命中时回退 MusicBrainz/Cover Art Archive；左侧依次为候选表、歌词正文和同步播放器，右侧通过“封面候选 / 音频标签”页签切换，可编辑标题、歌手、专辑、年份和轨道号；相同搜索缓存 10 分钟。下载同步歌词会先备份已有 LRC；独立的歌词核对页保留本地时间戳，只校准歌词文字。
“音频编辑”采用按工具切换的独立工作台：裁剪拥有精确选区和提取/删除片段模式；降噪拥有处理前后双波形；均衡器拥有八段推子；拼接与混音拥有轨道、静音、独奏、分轨音量和左右声道合成；分段、增益、响度、变速变调、提取与标签也各自使用专属界面。波形后台生成并缓存，隐藏工具按需加载且不跟随播放头重绘。结果始终保存为新文件并可进入手机待回传流程。
