...
HELP_SECTIONS = (
    ...
    (
        ...
        中间是可编辑波形时间轴，右侧显示所选效果的处理范围和参数。</p>
        <ul>
          <li><b>选区：</b>拖动波形，或精确输入起点、终点；滚轮和工具栏用于缩放。</li>
          <li><b>编辑：</b>裁剪与淡化、分段导出和增益。</li>
          <li><b>音质：</b>FFT 降噪、响度标准化、三段均衡器和变速变调。</li>
          <li><b>合成：</b>顺序拼接、多轨混合、提取音轨和常用音乐标签。</li>
        </ul>
        <p>支持选区的效果在有选区时只生成选区结果，否则处理整段。默认输出到素材旁的
