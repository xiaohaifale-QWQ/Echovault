...
HELP_SECTIONS = (
    (
        ...
        <ul>
          ...
          <li><b>音频编辑：</b>裁剪、拼接、混音、降噪、变速变调、标签和格式转换。</li>
    ...
    (
        ...
        """
        <h2>音频编辑工作区</h2>
        <p>选择音乐或视频素材后打开右侧“音频编辑”。首页按用途排列工具，
        点击后进入独立参数页；输入、输出、试听和执行进度都在同一页完成。</p>
        <ul>
          <li><b>剪辑：</b>提取音轨、精确裁剪、淡入淡出、拼接、混音和固定时长分割。</li>
          <li><b>处理：</b>FFT 降噪、LUFS 响度归一化、三段均衡器和分贝音量增益。</li>
          <li><b>变换：</b>独立调整速度/音高，以及格式、声道、采样率和倒放。</li>
          <li><b>文件：</b>麦克风录音、常用音乐标签、当前素材信息和本次输出清单。</li>
        </ul>
        <p>默认输出到素材旁的 <code>Echovault编辑输出</code>，不会覆盖原文件。
        手机传入的素材编辑完成后，会自动进入“待回传”目录并刷新传输结果。</p>
