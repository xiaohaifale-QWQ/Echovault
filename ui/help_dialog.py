...
HELP_SECTIONS = (
    ...
    (
        ...
        在线页位于最前，点击“一键搜索歌词与封面”会并行查询 LRCLIB 和 MusicBrainz；
        左侧显示在线歌词，右侧显示封面与音频标签，播放器会同步高亮并滚动歌词。</p>
        ...
        <p>歌词与封面同时可见；相同搜索会缓存 10 分钟，封面缩略图并发加载。
        也可选择本地 JPEG/PNG。右侧音频标签可直接修改标题、歌手、专辑、年份和
    ...
    (
        ...
        <ul>
          ...
          <li><b>输出：</b>分段、提取和音乐标签均有独立操作界面。</li>
        </ul>
        <p>默认输出到素材旁的
